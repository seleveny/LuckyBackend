...
CREATE TABLE IF NOT EXISTS `lottery_double_ball` (
    ...
    `pool_amount` DECIMAL(14, 2) DEFAULT 0.00 COMMENT '奖池金额（元）',
    `first_prize_count` INT DEFAULT 0 COMMENT '一等奖注数',
    `first_prize_amount` DECIMAL(12, 2) DEFAULT 0.00 COMMENT '一等奖单注金额（元）',
    `second_prize_count` INT DEFAULT 0 COMMENT '二等奖注数',
    `second_prize_amount` DECIMAL(12, 2) DEFAULT 0.00 COMMENT '二等奖单注金额（元）',
    `sales_amount` DECIMAL(14, 2) DEFAULT 0.00 COMMENT '本期销售额（元）',
